    ...
    <main class="relative mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
        <div class="glass-card rounded-3xl p-6 sm:p-8 md:p-10 fade-up">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <span class="pill rounded-full px-3 py-1 text-xs font-semibold">Hasil Kamu</span>
                <span class="text-xs text-soft">Analisa ngawur tapi niat</span>
            </div>

            <h1 class="mt-6 text-2xl font-bold text-ink sm:text-3xl md:text-4xl">
                "Wah {{ $nama }}, kamu {{ $dominantTitle }} banget!"
            </h1>
            <p class="mt-4 text-sm italic text-muted sm:text-base">
                @if($dominantTraitKey == 'jomok')
                    Hati-hati, jangan terlalu sering natap temen sendiri.
                @elseif($dominantTraitKey == 'femboy')
                    Stok rok mini aman bang?
                @elseif($dominantTraitKey == 'pedo' || $dominantTraitKey == 'psikis')
                    Segera cari bantuan profesional, atau minimal ruqyah.
                @elseif($dominantTraitKey == 'introvert')
                    Keluar kamar bang, matahari itu nyata.
                @else
                    Tumben ada manusia normal main ginian.
                @endif
            </p>

            <div class="mt-6 sm:mt-8">
                <h3 class="text-base font-semibold text-ink sm:text-lg">Rincian Statistik Jiwa Kamu</h3>
                <div class="mt-4 space-y-4">
                    @foreach($percentages as $cat => $percent)
                    <div class="rounded-2xl border border-soft bg-card-strong p-4 sm:p-5">
                        <div class="flex items-center justify-between text-sm font-semibold">
                            <span class="text-ink">{{ $categoryLabels[$cat] ?? ucfirst($cat) }}</span>
                            <span class="text-soft">{{ $percent }}%</span>
                        </div>
                        <div class="mt-3 h-2.5 w-full rounded-full bg-accent-soft">
                            @php
                                $color = 'bg-fuchsia-500';
                                if($cat == 'jomok') $color = 'bg-purple-600';
                                if($cat == 'femboy') $color = 'bg-pink-500';
                                if($cat == 'pedo' || $cat == 'psikis') $color = 'bg-violet-700';
                                if($cat == 'introvert') $color = 'bg-fuchsia-600';
                                if($cat == 'normal') $color = 'bg-rose-400';
                            @endphp
            ...
            <div class="mt-8 text-center">
                <a href="{{ route('home') }}" class="btn-primary inline-flex items-center justify-center rounded-xl px-5 py-3 text-sm font-semibold text-white transition sm:px-6 sm:text-base">
                    Coba Lagi (Siapa tau berubah)
                </a>
            </div>
        </div>
    </main>
